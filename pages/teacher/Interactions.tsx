...
const Interactions: React.FC<InteractionsProps> = ({
  ...
  // 讨论区状态
  const [, setSelectedTopic] = useState<DiscussionTopic | null>(null);
  const [, setShowTopicDetail] = useState(false);
  ...
  const [, _setSelectedNotification] = useState<Notification | null>(null);
  ...
  const [topics, setTopics] = useState<DiscussionTopic[]>([
    ...
    },
    {
      ...
      content: '我们团队正在进行敏捷转型，遇到了一些困难，想和大家交流一下。',
  ...
  const filteredTopics = topics.filter(t => {
    if (topicFilter === 'pinned') return t.isPinned;
    if (topicFilter === 'active') return t.repliesCount > 10;
    return true;
  });
