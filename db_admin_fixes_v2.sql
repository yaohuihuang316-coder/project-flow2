...
SELECT name, description, category, icon, is_active, required_tier, difficulty 
FROM (VALUES
    ...
    ('鱼骨图分析', '结构化根因分析，人机料法环五维诊断', 'risk', 'AlertTriangle', true, 'pro_plus', 'Medium'),
    ('质量成本模型', 'COQ分析 - 预防、评估与失败成本优化', 'evm', 'DollarSign', true, 'pro_plus', 'Medium')
) AS v(name, description, category, icon, is_active, required_tier, difficulty)
WHERE NOT EXISTS (SELECT 1 FROM app_tools LIMIT 1);

-- ==========================================
-- 脚本执行完成
-- ==========================================
SELECT '后台管理修复完成 V2' as status;
