...
async function setupRLS() {
    console.log('配置正确的RLS策略...\n');
    
    // 1. app_user_skills - 用户只能查看和修改自己的数据
    ...
    await supabase.rpc('exec_sql', {
        sql: `
            ...
            ON app_user_skills FOR SELECT
            USING (user_id = auth.uid()::text);
            
            CREATE POLICY "Users can insert own skills"
            ON app_user_skills FOR INSERT
            WITH CHECK (user_id = auth.uid()::text);
            
            CREATE POLICY "Users can update own skills"
            ON app_user_skills FOR UPDATE
            USING (user_id = auth.uid()::text);
        `
    });
    console.log('   ✅ 完成');
    
    // 2. app_user_achievements - 用户只能查看和修改自己的数据
    ...
    await supabase.rpc('exec_sql', {
        sql: `
            ...
            ON app_user_achievements FOR SELECT
            USING (user_id = auth.uid()::text);
            
            CREATE POLICY "Users can insert own achievements"
            ON app_user_achievements FOR INSERT
            WITH CHECK (user_id = auth.uid()::text);
        `
    });
    console.log('   ✅ 完成');
    
    // 3. app_learning_activity - 用户只能查看和修改自己的数据
    ...
    await supabase.rpc('exec_sql', {
        sql: `
            ...
            ON app_learning_activity FOR SELECT
            USING (user_id = auth.uid()::text);
            
            CREATE POLICY "Users can insert own activity"
            ON app_learning_activity FOR INSERT
            WITH CHECK (user_id = auth.uid()::text);
        `
    });
    console.log('   ✅ 完成');
    
    // 4. app_achievements - 所有人都可以查看成就定义
    ...
    await supabase.rpc('exec_sql', {
        sql: `
            ...
            ON app_achievements FOR SELECT
            USING (true);
        `
    });
    console.log('   ✅ 完成');
